...
async function scrapeInterior(mapsUrl) {
...
async function scrapeWebsite(websiteUrl) {
  ...
  try {
    ...
    const pageData = await page.evaluate((baseUrl) => {
      ...
      function findSectionByHeading(keywords) {
        ...
        for (const h of headings) {
          ...
          for (const kw of keywords) {
            if (text.includes(kw)) {
              if (kw === 'designer' && text.includes('designerry')) continue
          ...
          if (matches) {
            if (keywords.includes('about')) {
              if (text.includes('faq') || text.includes('question') || text.includes('services') || text.includes('service') || text.includes('testimonials')) {
                continue
              }
            }
      ...
      // ── 1. SERVICES ──
      const svcSection = getSectionByIdOrHeading(['services', 'service', 'solution'], ['service', 'service', 'what we offer', 'specialit', 'solution'])
      if (svcSection) {
        const excludeList = ['home', 'about', 'services', 'testimonials', 'contact', 'appointment', 'book', 'schedule', 'gallery', 'blog', 'privacy', 'terms', 'careers', 'team', 'team', 'faq', 'faqs', 'map', 'location', 'read more', 'learn more', 'view more', 'submit', 'send', 'solution', 'problem']
      ...
      // ── 2. DOCTORS / TEAM ──
      const teamSection = getSectionByIdOrHeading(['team', 'doctor', 'staff', 'expert'], ['team', 'doctor', 'specialist', 'designer', 'our expert', 'meet our', 'staff', 'physician'])
      if (teamSection) {
        console.log('  [Doctors] Section found!')
        const candidates = extractCardsFromSection(teamSection, ['team', 'expert', 'our team', 'meet our', 'specialist', 'designer'])
        candidates.forEach((c, index) => {
          ...
          const hasNonPersonWord = [
            ...
            'imaging', 'cbct', 'stories', 'story', 'solutions', 'solution', 'implants',
            'implant', 'tour', 'virtual', 'services', 'service', 'blog', 'recent', 'designerry',
            'orthodontic', 'service', 'whitening', 'cleanup', 'clean', 'braces', 'aligner',
            ...
            'excellence', 'care', 'studio', 'center', 'studio', 'agency'
          ...
          const hasForbiddenWord = words.some(w => [
            ...
            'better', 'experience', 'comfort', 'client', 'clients', 'reason', 'reasons',
            'standard', 'standards', 'safety', 'beautiful', 'success', 'rate', 'rates',
            'result', 'results', 'record', 'records', 'transparency', 'honest', 'counsel',
            'solutions', 'fixing', 'braces', 'visit', 'scanning', 'crowns', 'implants',
            'technology', 'appointment', 'contact', 'schedule', 'learn', 'more', 'read', 'view',
            'specialist', 'specialists', 'designer', 'designers', 'doctor', 'team', 'staff',
            'expert', 'experts', 'excellence', 'care', 'studio', 'center', 'studio', 'agency'
          ].includes(w))
          ...
          const isDoctor = (nameLower.startsWith('dr') || nameLower.includes('designer') || nameLower.includes('architect') || (c.name.split(' ').length >= 2 && c.name.length < 40)) && 
          ...
          if (isDoctor && isNotBtn) {
            ...
            data.team.push({
              name: c.name,
              role: c.description || 'interior Specialist',
              photo: c.image
            })
          }
        })
      ...
      // Fallback team: scan entire page text for "Dr. [Name]" patterns if none found in team section
      if (data.team.length === 0) {
        ...
        matches.forEach(m => {
          ...
          const hasInvalidWord = nameWords.some(w => [
            'Everything', 'Follow', 'We', 'Our', 'The', 'If', 'You', 'In', 'On', 'At', 'By', 'To', 'This', 'That', 'It', 'They', 'He', 'She', 'So', 'Thus', 'Because', 'For', 'And', 'But', 'Or', 'Please', 'All', 'Get', 'Book', 'Contact', 'Welcome', 'Read', 'More', 'View', 'Learn', 'Best', 'interior', 'studio', 'agency', 'Care', 'Expert', 'Specialist', 'Dentist', 'Co', 'Inc', 'Ltd', 'Corp', 'Group'
          ].includes(w))

          const hasDigits = /\d/.test(name)

          if (!seenNames.has(nameNormalized) && name.length < 40 && !hasInvalidWord && !hasDigits && !nameLower.includes('studio') && !nameLower.includes('agency') && !nameLower.includes('interior')) {
        ...
        allImgs.forEach(img => {
          ...
          if (src && !src.includes('icon') && !src.includes('logo') && !src.includes('arrow') && !src.includes('banner') && !src.includes('header')) {
            if (img.width > 200 || img.height > 200 || img.alt) {
              ...
              if (!seenUrls.has(urlLower) && (urlLower.includes('gallery') || urlLower.includes('upload') || urlLower.includes('interior') || urlLower.includes('studio') || urlLower.includes('service') || urlLower.includes('before') || urlLower.includes('after'))) {
      ...
      // ── 4. TESTIMONIALS ──
      const testiSection = getSectionByIdOrHeading(['testimonial', 'review', 'feedback'], ['testimonial', 'review', 'client say', 'feedback', 'what our', 'happy client'])
      if (testiSection) {
        console.log('  [Testimonials] Section found!')
        const candidates = extractCardsFromSection(testiSection, ['testimonials', 'reviews', 'what our', 'happy client', 'transformations', 'satisf', 'read', 'our process', 'solutions'])
      ...
      allTextElements.forEach(el => {
        ...
        if (/^\d+(\.\d+)?\s*(\+|★|years)?$/i.test(text) && text.length > 0 && text.length < 10) {
          ...
          if (labelText.includes('client') || labelText.includes('happy client') || labelText.includes('satisfied')) {
            if (!data.stats.clients) data.stats.clients = text
    ...
    const subpageLinks = await page.evaluate((baseUrl) => {
      ...
      allAnchors.forEach(a => {
        ...
        if (href.startsWith(baseUrl) || href.startsWith('/')) {
          ...
          if ((text.includes('service') || text.includes('service') || href.includes('/service') || href.includes('/service')) && !links.services) links.services = href
    ...
    for (const [type, url] of Object.entries(subpageLinks)) {
      ...
      if (type === 'team' && result.team.length >= 1) continue
      ...
      try {
        ...
        const subData = await page.evaluate((baseUrl) => {
          const data = { services: [], team: [], gallery: [], testimonials: [], about: '' }
          ...
          // Services
          const svcCards = document.querySelectorAll('[class*="service"], [class*="service"], [class*="card"]')
          ...
          docCards.forEach(card => {
            ...
            if (name && name.length > 3 && name.length < 80 && !seenDoc.has(name)) {
              ...
              const isDoc = (nameLower.startsWith('dr') || nameLower.includes('designer') || nameLower.includes('architect') || (name.split(' ').length >= 2 && name.length < 40)) && !hasNonPersonWord
          ...
          testiCards.forEach(card => {
            ...
            if (quote && quote.length > 20 && !seenT.has(quote.slice(0, 50))) {
              ...
              data.testimonials.push({
                name: nameEl?.textContent?.trim() || 'client',
                role: '', photo: '',
        ...
        if (type === 'team' && subData.team.length > result.team.length) result.team = subData.team
  ...
  console.log('  [Website] Got:', result.services.length, 'services,', result.team.length, 'team,',
...
if (require.main === module) {
  ...
  const fn = mode === 'website' ? scrapeWebsite : scrapeInterior
  fn(url)
    .then(data => {
      console.log('\n--- SCRAPED DATA ---')
...
module.exports = { scrapeInterior, scrapeWebsite, generateSlug }

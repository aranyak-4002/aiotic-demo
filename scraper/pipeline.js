...
function parseArgs() {
  ...
  const opts = { mapsUrl: '', city: '', template: 'clinic', dryRun: false }
...
`)
  ...
  const mapsData = await scrapeClinic(opts.mapsUrl)
  ...
  const record = await upsertToSupabase(slug, opts.template, data.name, opts.mapsUrl, data)
  console.log(`  ✓ Saved! ID: ${record.id}`)

  const demoUrl = `https://demo.getaiotic.com/${opts.template}/?client=${encodeURIComponent(slug)}`
  console.log(`\n━━━ Demo URL ━━━`)
  console.log(`  ${demoUrl}`)
  console.log('')
}
